...
public class BasicCameraRotation : MonoBehaviour
{
    ...
    {
        transform.RotateAround(player.transform.position, transform.right, -axis * Time.deltaTime);
    }

    //rotate the camera rigt and left (y rotation)
    public void RotateRightLeft(float axis)
    {
        transform.RotateAround(player.transform.position, Vector3.up, -axis * Time.deltaTime);
    }
}